...
dataset_path='Maskdata'
output_file='Maskdata/train_mask.tfrecord'
...
def main():
    ...
    if os.path.exists(output_file):
        logging.info('{:s} already exists. Exit...'.format(output_file))
        exit()

    with tf.io.TFRecordWriter(output_file) as writer:
        img_list = open(
            os.path.join(dataset_path, 'ImageSets', 'Main', 'train.txt')).read().splitlines()
        logging.info('Image list loaded: %d', len(img_list))
        counter = 0
        skipped = 0
        for image in tqdm.tqdm(img_list):
            image_file = os.path.join(dataset_path, 'JPEGImages', '%s.jpg' % image)
            annot_file = os.path.join(dataset_path, 'Annotations', '%s.xml' % image)
